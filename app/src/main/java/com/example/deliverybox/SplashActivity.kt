...
/**
 * 스플래시 화면 및 앱 시작 로직 처리
 * - 네트워크 연결 확인
 * - 로그인 상태 및 계정 유효성 확인
 * - 회원가입 단계 확인 및 적절한 화면으로 이동
 */
class SplashActivity : AppCompatActivity() {
    ...
    private fun checkLoginStatus() {
        ...
        if (currentUser != null) {
            ...
            // 미인증 계정 처리 - 오래된 미인증 계정은 삭제
            if (!currentUser.isEmailVerified) {
                // 인증되지 않은 이메일
                Log.d(TAG, "이메일 미인증 계정: ${currentUser.email}")

                // 임시 계정 유효성 확인
                AccountUtils.checkTempAccountValidity(uid) { isValid ->
                    if (isValid) {
                        // 유효 기간 내 미인증 계정 - 인증 화면으로
                        navigateToEmailVerification(currentUser.email ?: "")
                    } else {
                        // 유효 기간 초과 - 계정 삭제 후 로그인 화면으로
                        Log.d(TAG, "기간 만료된 미인증 계정, 삭제 처리")
                        AccountUtils.deleteTempAccountAndSignOut {
                            navigateToLogin()
                        }
                    }
            ...
            db.collection("users").document(uid)
                ...
                .addOnSuccessListener { doc ->
                    ...
                    if (!isPasswordSet) {
                        // 비밀번호 설정 필요
                        Log.d(TAG, "비밀번호 설정 필요: ${currentUser.email}")
                        navigateToPasswordSetting(currentUser.email ?: "")
                ...
                .addOnFailureListener { e ->
                    ...
                    Log.e(TAG, "사용자 데이터 로드 실패, 로그아웃: $uid, 오류: ${e.message}")
                    auth.signOut()
                    SharedPrefsHelper.clearLoginSession(this)
     ...
     * 비밀번호 설정 화면으로 이동
     */
    private fun navigateToPasswordSetting(email: String) {
    ...
    override fun onResume() {
        ...
        // 네트워크 상태 재확인
        if (!NetworkUtils.isNetworkAvailable(this) && binding.tvNetworkError.visibility != View.VISIBLE) {
            showNetworkError()
        }
    }
}